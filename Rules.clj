(t:Extension: From Goal Backward 1
...
(t:Extension: From Goal Forward 1
(Work status "EXTENSION")
(GoalR a ?a b ?b min-length ?mil)
?t (Trail1 status "OPEN"
	a ?a
...
(t:Check: Max Solutions -> Optimization 1
?w (Work status "CHECK")
...
?a (Stop status "START"
	...
	next_actions ?nacts)
=>
(if s/TRACE (println "Action started:" ?tit "Stop"))
(ru.rules/confirm ?txt)
(retract ?a)
(s/start-next ?nacts ?pid ?ain ?run))

(t:Extension: Goal Behind Forward 1
(Work status "EXTENSION")
(GoalR a ?a b ?b min-length ?mil)
?t (Trail1 status "OPEN"
	a ?a
...
(t:Extension: Goal Behind Backward 1
(Work status "EXTENSION")
(GoalR a ?a b ?b min-length ?mil)
?t (Trail1 status "OPEN"
	a ?b
...
(t:Extension -> Quasioptimisation -1
?w (Work status "EXTENSION")
(Quasisolution)
(not Solution)
...
(t:Quasioptimization: Nearest Quasisolution 0
(Work status "QUASIOPTIMIZATION")
(GoalR a ?a b ?b)
...
?qs2 (Quasisolution path ?pth2
	...
	b ?c2)
=>
(osm.path/if-display-edges "Quasioptimization between" ?pth1)
(osm.path/if-display-edges "                    and" ?pth2)
(if (< (osm.func/simple-dist ?b ?c1) (osm.func/simple-dist ?b ?c2))
  (retract ?qs2)
  (retract ?qs1)))

(t:Quasioptimization: Quasisolutions optimized 0
(Work status "QUASIOPTIMIZATION")
(GoalR title ?tit a ?a)
...
(trn:GoDirectedRoadStart 0
?gor (GoDirectedRoad status "START"
	title ?tit
	object ?obj
	road ?road
	spd ?spd
	...
	parent ?par)
=>
(if s/TRACE (println "Action started:" ?tit "GoDirectedRoad"))
...
(trn:GoDirectedRoadWait 0
?gor (GoDirectedRoad status "WAIT"
	object ?mo
	road ?dws
	direction ?gbw
	parent ?pid
...
(t:Check: Two Trail1 Connection 0
(Work status "CHECK")
?g (GoalR status "RUN"
	a ?a
	b ?b)
...
?t (Trail0 status "OPEN"
	...
	last ?e)
=>
(if-let [nois (seq (filter some?(osm.path/find-new-nodes ?b)))]
...
(t:Check: Longer Trail1 Retract 0
(Work status "CHECK")
(Solution path ?pth)
?t (Trail1 edges ?egs
	(> (count ?egs) (count ?pth)))
...
(t:Optimization: Solutions Optimization 0
...
?s2 (Solution path ?pth2
	...
	b ?b)
=>
(osm.path/if-display-edges "Optimization between" ?pth1)
(osm.path/if-display-edges "            and" ?pth2)
...
(t:Optimization: Solutions Optimized 0
(Work status "OPTIMIZATION")
(GoalR title ?tit a ?a)
(Solution path ?pth)
(not Solution)
...
(t:Extension -> No Any Solutions -1
?w (Work status "EXTENSION")
(not Quasisolution)
(not Solution)
(not Trail1 status "OPEN")
...
?na (NoAction status "START"
	...
	next_actions ?nacts)
=>
(if s/TRACE (println "Action started:" ?tit "NoAction"))
(retract ?na)
(s/start-next ?nacts ?pid ?ain ?run))
...
(let [mo (a/mapob-vv ?obj ?run)
       scl (a/vv ?scl ?run)
       del (a/vv ?del ?run)]
  (if (and (some? mo) (a/worth? scl) (a/worth? del))
    (let [mb (ru.igis.omtab.OpenMapTab/getMapBean)]
      (.setScale mb (float (read-string scl)))
...
?tm (CalcMessage status "START"
	...
	next_actions ?nacts)
=>
(if s/TRACE (println "Action started:" ?tit "CalcMessage"))
(let [cat (a/vv ?cat ?run)
       bnd0 (d/embed-ctx-vars ?run)
       bnd2 (d/parse-let-body (d/uncomment ?txt))
       bnd3 (vec (concat bnd0 bnd2))
       lvr (last (butlast bnd3))
       sex (str "(in-ns 'a) (let " bnd3 lvr ")")
       ;;_ (println :SEX sex)
       txt (str (load-string sex))]
  (a/es-mess (str (a/op-time) " " txt) {} cat))
(retract ?tm)
(s/start-next ?nacts ?pid ?ain ?run))
...
?pmn (PolyMovingN status "REPEAT"
	...
	next_actions ?nacts)
(Clock time ?t (> ?t ?clk))
(not PolyMoving status "REPEAT"
	title ?tit
	object ?mob
	run ?run
	(= (a/mapob-vv ?obj ?run) ?mob))
=>
(let [rmpb (rest ?mpb)]
  (if (empty?(rest rmpb))
    (do (retract ?pmn)
      (s/start-next ?nacts ?pid ?ain ?run))
    (do (asser PolyMoving status "START"
	title ?tit
	object ?obj
	observer (first rmpb)
	observer2 (second rmpb)
	time ?tim
	run ?run)
      (modify ?pmn mapob rmpb
	clock ?t)))))
...
(Clock time ?t)
=>
(println (str "Action started: " ?tit " PolyMovingN"))
(let [kit (a/vv ?kit ?run)
      col (if kit (protege.core/svs kit "collection") [])
      mpb (concat ?mpb col)]
  (asser PolyMoving status "START"
	title ?tit
	object ?obj
	observer (first mpb)
	observer2 (second mpb)
	time ?tim
	default ?def
	run ?run)
  (modify ?pmn status "START2"
	mapob mpb
	clock ?t)))

(a:PolyMovingNStart2 0
?pmn (PolyMovingN status "START2"
	title ?tit
	object ?obj
	run ?run)
(PolyMoving status "REPEAT"
	title ?tit
	object ?mob
	run ?run
	(= (a/mapob-vv ?obj ?run) ?mob))
...
?pm (PolyMoving status "START"
	...
	run ?run)
=>
(println (str "Action started: " ?tit " PolyMoving"))
(protege.core/ssvs ?obj "points" (protege.core/svs ?obs "points"))
(let [mob (a/mapob-vv ?obj ?run)
       mos (a/mapob-vv ?obs ?run)
       mos2 (a/mapob-vv ?obs2 ?run)
       tim (a/vv ?tim ?run)
       def (a/vv ?def ?run)
       pmm (ru.igis.omtab.PolyMovingManager/getPolyMovingManager)]
  (if (and (some? mob) (some? mos) (some? mos2) (a/worth? tim) pmm)
    (do (if (some? def)
           (let [def (read-string def)]
             (when (not= (.getTimerDelay pmm) def)
               (.setDefaultTimerDelay pmm def)
               (.resetTimer pmm def))))
      (modify ?pm status "REPEAT"
	object mob
	observer (poly.moving/poly-mover mob mos2 (a/to-be tim) pmm)))
...
(let [mo (a/mapob-vv ?obj ?run)
       ...
       num (a/vv ?num ?run)
       kit (a/vv ?kit ?run)
       mos (concat ?mos (protege.core/svs kit "collection"))
       rr (cond
            thg
            (a/any-collection thg)
            mo
            (or (seq mos) 
