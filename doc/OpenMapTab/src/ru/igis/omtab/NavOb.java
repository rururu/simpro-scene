...
public class NavOb extends OMTRaster {
    ...
    public static NavOb createNavOb(
        ...
        if(cls!=null){
            ...
            if(inst!=null){
                ...
                inst.setOwnSlotValue(OpenMapTab.kb.getSlot(Ontology.S_ALTITUDE),Integer.getInteger(altitude, 0));
                inst.setOwnSlotValue(OpenMapTab.kb.getSlot(Ontology.S_COURSE),Integer.getInteger(course, 0));
	...
	public synchronized void setCourse(int deg) {
		if (nextPointIndex > 0) {
			if (speed > 0) {
				// 1 hours way
				double[] ohw = position(lat, lon, (double) deg, speed);
				goRoute(ohw);
			}
		}
		setCourse0(deg);
	...
	public synchronized void setSpeed(double knots) {
		if (nextPointIndex > 0 && knots > 0) {
			...
			if (nextPointIndex == route.size()) {
				// 1 hour way
				double[] ohw = position(lat, lon, (double) course, knots);
				goRoute(ohw);
